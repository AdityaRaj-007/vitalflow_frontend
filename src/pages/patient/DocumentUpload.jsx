...
const INITIAL_FILES = [
  { name: 'blood_panel_jan2026.pdf', size: '2.4 MB', type: 'Lab Results',  status: 'processed',  date: 'Jan 15' },
  { name: 'ecg_report_dec2025.pdf',  size: '1.1 MB', type: 'Cardiology',   status: 'processed',  date: 'Dec 28' },
  { name: 'rx_metformin.pdf',        size: '340 KB', type: 'Prescription', status: 'processing', date: 'Feb 20' },
]
...
export default function DocumentUpload() {
  ...
  const simulateUpload = () => {
    const newFile = { name: `document_${Date.now()}.pdf`, size: '1.8 MB', type: 'General', status: 'uploading', date: 'Feb 25' }
          ...
          <Card>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea className="form-input resize-none h-20" placeholder="Add context about this document…" />
              </div>
              <Button icon="upload" className="w-fit">Upload File</Button>
            </div>
          </Card>
        </div>
            ...
            {files.map((f, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] sm:text-[11px] text-slate">{f.type} · {f.size} · {f.date}</p>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  {f.status === 'processed' && (
                    <button className="btn-ghost !p-1.5"><Icon name="eye" size={13} /></button>
                  )}
                </div>
              </div>
            ))}
